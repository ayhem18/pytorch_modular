...
class TestConvBlockDesignHelper(unittest.TestCase):
    ...
    def test_compute_all_possible_inputs(self):
        ...
        possible_output_dims = list(range(1, 100, 5))


        for output_dim in tqdm(possible_output_dims, desc="Testing output dimensions"):
            # Compute possible input dimensions for 1 and 2 blocks
            res = set()
            memo = set()
            
            
            compute_all_possible_inputs(min_n, max_n, output_dim, res, memo, 5)

            # remove the output_dim itself from the memo
            memo.remove(output_dim)
    ...
    def test_compute_impossible_inputs(self):
        ...
        for output_dim in tqdm(possible_output_dims, desc="Testing impossible inputs"):
            ...
            compute_all_possible_inputs(min_n, max_n, output_dim, possible_inputs, memo, 3)
