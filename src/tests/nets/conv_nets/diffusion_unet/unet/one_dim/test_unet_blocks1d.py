...
class TestUnetUpBlock1D(CustomModuleBaseTest):
    ...
    def _get_valid_input_and_skip(self, 
        ...
        skip_outputs: List[torch.Tensor] = [torch.tensor(0) for _ in range(num_up_layers)] 
        ...
        for i in range(num_up_layers):
            ...
            skip_channels = block.skip_connections_channels[i] 
    ...
    def _generate_random_up_block(self,
                                 ...
                                 cond_dimension=None,
                                 out_channels=None,
                                 skip_connections_channels=None,
                                 reverse_skip_connections=None,
                                 upsample_types=None,
                                 dropout_rate=None,
        ...
        if skip_connections_channels is None:
            skip_connections_channels = [random.randint(*self.in_channels_range) for _ in range(num_up_layers)]
        ...
        return UnetUpBlock1D(
            ...
            cond_dimension=cond_dimension,
            out_channels=out_channels,
            skip_connections_channels=skip_connections_channels,
            reverse_skip_connections=False,
