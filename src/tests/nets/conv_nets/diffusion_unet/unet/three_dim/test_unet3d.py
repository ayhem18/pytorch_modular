...
class TestUNet3DCond(CustomModuleBaseTest):
    ...
    def setUp(self):
        ...
        self.num_resnet_blocks_range = (2, 4)  # number of resnet blocks must be at least 2
    ...
    def _create_unet_model(self, 
                          ...
                          cond_dimension: Optional[int] = None) -> UNet3DCond:
        """Create a basic UNet3DCond model without building it"""
    ...
    def _get_valid_input(self, 
        ...
        # For 3D UNet, condition has spatial dimensions matching input
        condition = torch.randn(batch_size, model.cond_dimension, spatial_size, spatial_size)
        
        return x, condition
    
    def _build_complete_model(self, model: Optional[UNet3DCond] = None) -> UNet3DCond:
    ...
    def test_down_block_structure(self):
        ...
        for _ in range(20):
            ...
            for i, layer in enumerate(model._down_block.down_layers):
                # the layer.out_channels is a list (modified inside the UnetDownLayer3D)
    ...
    def test_middle_block_structure(self):
        ...
        for _ in range(20):
            ...
            # Check attributes are set correctly
            self.assertEqual(model.middle_block_num_resnet_blocks, mid_block_num_resnet_blocks)
            
            # Check middle block structure
            self.assertEqual(len(model._middle_block._mid_blocks), mid_block_num_resnet_blocks)
    ...
    def test_up_block_structure(self):
        ...
        for _ in range(20):
            ...
            # Check up block structure
            self.assertEqual(len(model._up_block.up_layers), num_down_layers)
            
            # Check the input channels of the up block
            self.assertEqual(model._up_block.in_channels, out_channels[-1])
            
            # Check the output channels of the last up layer
            # the layer.out_channels is a list (modified inside the UnetUpLayer3D)
    ...
    def test_spatial_condition_handling(self):
        """Test that the model correctly handles spatial conditioning"""
        model = self._build_complete_model()
        
        # Generate input and condition with different spatial dimensions
        x, _ = self._get_valid_input(model)
        
        # create a condition with a different spatial size
        different_condition = torch.randn(x.shape[0], model.cond_dimension, random.randint(1, x.shape[2] - 1), random.randint(1, x.shape[3] - 1))

        # Forward pass should raise an error if the condition does not match the input spatial dimensions
        try:
            model(x, different_condition)
        except Exception as e:
            self.assertIn("The input tensor and the condition tensor should have the same height and width dimensions.", str(e))
            return
    ...
    def test_invalid_input_dimensions(self):
        ...
        condition = torch.randn(batch_size, model.cond_dimension, invalid_size, invalid_size)
