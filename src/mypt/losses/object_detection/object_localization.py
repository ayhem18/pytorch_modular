...
class ObjectLocalizationLoss(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor, all: bool=False) -> Union[torch.Tensor, Dict]:
        ...
        l2 = torch.sum(torch.nn.MSELoss(reduction=self.reduction).forward(bounding_boxes_pred[y_index], y_bbox[y_index]), dim=1, keepdim=True) # the loss on the bounding boxes will be the sum of mse on the four corners
        ...
        final_loss = torch.mean(l1, dim=0) + torch.mean(l2, dim=0) + torch.mean(l3, dim=0)
        
        if all:
            return {self._loss_name: final_loss, self._l1_name: l1, self._l2_name: l2, self._l3_name: l3}
